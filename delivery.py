...
if domain == "":
    ...
    with open("domain.txt","w") as f:
        ...
        f.close()
while True:
    state = input("Enter delivery state: ").strip().upper()
    if state in states:
        break
    else:
...
while True:
    ...
    if sleep_time == "":
        try:
            with open("delivery_sleep.txt","r") as f:
        ...
        try:
            ...
            sleep_time = float(sleep_time)
            with open("delivery_sleep.txt","w") as f:
...
bot = Car_bot(domain,"delivery_database.db",sleep_time)
bot.login("username","password")
bot.select_jobs(state,"d_filter")
